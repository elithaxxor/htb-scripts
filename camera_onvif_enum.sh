...
function remote_scan() {
  read -rp "Camera IP: " IP
python3 - <<EOF
import asyncio
from onvif import ONVIFService
async def main():
    svc = ONVIFService('http://'+IP+'/onvif/device_service','admin','admin')
    info = await svc.devicemgmt.GetDeviceInformation()
    print(info)
asyncio.run(main())
EOF
...
function remote_ssh_execute() {
  ...
  scp "$0" "$user@$host:/tmp/camera_onvif_enum.sh"
  sshpass -p "$pass" ssh -o StrictHostKeyChecking=no "$user@$host" "bash /tmp/camera_onvif_enum.sh on-device"
}
